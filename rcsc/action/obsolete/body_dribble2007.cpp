...
Body_Dribble2007::execute( PlayerAgent * agent )
{
    dlog.addText( Logger::DRIBBLE,
                  __FILE__": Body_Dribble. to(%.1f, %.1f) dash_power=%.1f"
                  " dash_count=%d",
                                        ...
                                        const bool dodge_mode )
{
    ...
    const Rect2D pitch_rect( Vector2D( - ServerParam::i().pitchHalfLength() + 1.0,
                                       - ServerParam::i().pitchHalfWidth() + 1.0 ),
                             Size2D( ServerParam::i().pitchLength() - 2.0,
                                     ServerParam::i().pitchWidth() - 2.0 ) );
