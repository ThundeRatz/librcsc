...
/*!
  \file intention_dribble2006.cpp
...
*/
bool
IntentionDribble2006::finished( const PlayerAgent* agent )
{
...
*/
bool
IntentionDribble2006::execute( PlayerAgent * agent )
{
    if ( M_turn_step + M_dash_step == 0 )
    {
        dlog.addText( Logger::DRIBBLE,
    ...
    if ( ball_next.absX() > ServerParam::i().pitchHalfLength() - 0.15
         || ball_next.absY() > ServerParam::i().pitchHalfWidth() - 0.15 )
    {
        dlog.addText( Logger::DRIBBLE,
...
*/
bool
IntentionDribble2006::checkOpponent( const WorldModel & world )
{
...
*/
bool
IntentionDribble2006::doTurn( PlayerAgent * agent )
{
    if ( M_turn_step <=0 )
    {
        return false;
...
*/
bool
IntentionDribble2006::doDash( PlayerAgent * agent )
{
    if ( M_dash_step <=0 )
    {
        return false;
    ...
    Vector2D ball_next
        = wm.ball().rpos()
        + wm.ball().vel();
    double ball_next_dist = my_next.dist( ball_next );
    ...
    {
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: doDash. collision may occur."
                      ,__FILE__, __LINE__ );
        this->clear();
        return false;
    }
    ...
    if ( M_dash_step <= 0 )
    {
        ...
        {
            dlog.addText( Logger::DRIBBLE,
                          "%s:%d: doDash. last dash. but not kickable at next."
                          " ball_dist=%.3f"
                          ,__FILE__, __LINE__,
                          ball_next_dist );
            this->clear();
